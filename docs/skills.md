...
| ----- | ----- | ----------- |
| `system2-onboarding` | guide | First-launch setup: greets the user, learns about them, detects the system, configures the data stack (including external database connections and driver installation), sets up the development environment, and captures interaction preferences. Triggered when `infrastructure.md` is still the unedited template, or when the user explicitly asks to re-onboard. |
| `project-creation` | guide | Delegating complex work to a new project: gathers preliminary requirements with the user, creates the project in app.db, spawns a Conductor and Reviewer, introduces them to each other, and schedules a follow-up reminder so a silent Conductor is noticed. |
| `project-completion` | guide | Finalizing a completed project: confirms with the user, tells the Conductor to close the project, waits for the close-project report (including the Narrator's project story), then terminates the Conductor and Reviewer and marks the project done. |
...
| `statistical-analysis` | conductor, reviewer | Statistical methodology: frequentist and Bayesian workflows, test selection, assumption checking, effect sizes, multiple comparisons, power analysis, missing data handling, bootstrap methods, time series analysis, meta-analysis, reporting standards, and common anti-patterns. |
| `live-dashboard` | guide, conductor, reviewer, worker | Interactive HTML dashboard artifacts with live database queries via the postMessage bridge. Includes a copy-paste `queryDatabase` helper, workflow steps, Chart.js patterns, and a pre-flight checklist. |
| `code-review` | conductor, reviewer, worker | Code review checklist: repo-specific conventions, design/architecture, correctness, security (OWASP top 10), performance, SQL/data transformations, testing (including practical execution), readability, and labeled feedback format (issue/suggestion/question/nitpick/praise). |
| `reasoning-fallacy-review` | conductor, reviewer | Cognitive bias and reasoning fallacy detection: dual-process model, core biases (WYSIATI, confirmation, availability, anchoring, substitution, narrative fallacy, suppression of doubt), additional analytical biases, root deficiency analysis, and adversarial review techniques (premortem, Analysis of Competing Hypotheses, reference class forecasting). |
