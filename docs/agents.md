...
- **OAuth tier** (the primary tier — tried first): *one model per provider* applied to all roles. The resolver (`resolveOAuthModel` in [`src/shared/oauth-models.ts`](../src/shared/oauth-models.ts)) picks the latest entry in [pi-ai's catalog](https://github.com/badlogic/pi-mono/blob/main/packages/ai/src/models.generated.ts) matching a per-provider family regex (`^claude-opus-` for `anthropic`, `^gpt-\d+(\.\d+)?(-codex(-max|-spark)?)?$` for `openai-codex`, `^gpt-\d+(\.\d+)?$` for `github-copilot`), sorted by natural-version order. Date-snapshot ids are filtered out in favor of aliases. Falls back to a hardcoded id (`OAUTH_FALLBACKS`) if the catalog has no match. Override per provider in `~/.system2/auth/.auth.toml` with `[llm.oauth.<provider>] model = "..."`.
...
| `deliverMessage(content, details, urgent?)` | Reload knowledge, then send inter-agent message via `sendCustomMessage()`. Returns `Promise<void>` that resolves when the agent finishes processing (on `agent_end`) or rejects on permanent failure. Reload errors are swallowed to avoid dropping messages. Tracked in `pendingDeliveries` for failover replay. While a session reinit is in flight (post-scheduled-task reset or provider failover), calls queue into `pendingDeliveries` rather than rejecting — the replay paths deliver them against the new session once init completes. The uninitialized reject fires whenever no session is available and no reinit is in progress (before `initialize()` runs, or after a reinit attempt fails leaving `session=null` and `isReinitializing=false`). Each entry carries two watchdog timers (issue #194): `PENDING_DELIVERY_TIMEOUT_MS` (3 min) rejects entries that sit deferred without ever reaching the SDK (wedged-session guard); `DELIVERY_DISPATCH_TIMEOUT_MS` (6 min) rejects entries whose `sendCustomMessage` was invoked but for which `agent_end` never fired (silent SDK stream loss) and clears `busy` (normally owned by `agent_end`) so the wedged agent does not appear busy forever. Both are cleared automatically by every settlement path because the entry's `resolve`/`reject` are wrapped at push time. |
...
4. **Contaminated-turn guard.** If the failed turn already emitted model output (`message_start` / `message_update` / `tool_execution_start` fired before the error), reject all pending delivery promises with a transient error and clear `pendingDeliveries` *before* the retry/failover path runs. Resending an in-flight delivery would re-feed the same message to the model and re-run any tool side effects (e.g. file edits) that already committed before the connection dropped. The host tracks `currentTurnHasOutput` per turn (set on the output events above, reset on `turn_start` and `agent_end`). For scheduled deliveries the awaiter (e.g. `trackJobExecution`) marks the job failed, the cursor (`last_narrator_update_ts`) is not advanced, and the next cron tick redoes the window from scratch with the recipient's idempotency handling whatever partial work landed on disk. The OAuth-refresh, cooldown-rotation, and failover paths below still run normally (so a contaminating auth/client error still cooldowns the key and rotates providers); only the retry-with-sleep block short-circuits when there's nothing left to retry — entering it with empty `deliveriesToRetry` and no `pendingPrompt` would increment `retryAttempts` and await `sleep()` for a no-op, burning the budget for the next genuine error. If a `pendingPrompt` IS set, the retry path runs normally so the prompt gets re-sent — that has a different blast radius (a duplicate user-visible response, not state corruption) and is tracked separately
5. If retriable: wait with exponential backoff, retry with same provider (re-sends the failed prompt and/or all pending deliveries — but if step 4 cleared `pendingDeliveries`, only the prompt is retried)
6. If retries exhausted or immediate failover: mark key in cooldown, failover to next provider
7. Reinitialize the session with the new provider (`reinitializeWithProvider()`)
8. Replay pending deliveries (snapshot + any queued during the async reinit window) on the new session, then retry the pending prompt. Replay runs **before** the prompt retry so backlog entries are at the front of `pendingDeliveries` with their `deliverySendCount` accounted for before any await yields control — a concurrent `deliverMessage()` arriving during the prompt-retry await lands at the end of the queue, preserving FIFO order for `agent_end`'s shift logic
9. If all providers exhausted or `context_overflow` triggered by oversized deliveries: `pendingPrompt` is preserved on the `AgentHost`, but all pending delivery promises are rejected and `pendingDeliveries` is cleared with a clear error message; no automatic replay occurs when a provider later comes out of cooldown. For oversized deliveries specifically, the error is `Delivery dropped: message exceeded wire-size limits across all providers.`
...
The 50% split threshold matches the `reserveTokens` auto-compaction setting, leaving headroom for the system prompt, knowledge files, and compaction overhead. The result is a session with a compact summary of the safe history plus the recent tail. The tail is byte-bounded (`OVERFLOW_TAIL_BYTES_PER_TOKEN` × the split threshold): the split keys off message-level token `usage`, which injected `custom_message` deliveries don't carry, so an oversized-delivery-packed tail would otherwise stay over the window and re-overflow on every recovery. When the tail exceeds the budget it is dropped and recovery proceeds from the compacted head alone. The overflow-causing prompt is not retried; the agent resumes naturally on the next interaction. Pending deliveries (scheduled tasks, inter-agent messages) are replayed on the recovered session via `sendCustomMessage`, preserving them for the agent to process. Exception: when `context_overflow` is triggered by oversized deliveries (413 / 429 long-context errors classified as `context_overflow`), pending deliveries are dropped rather than replayed — re-sending the same payload across providers cannot shrink it. The producing caller's promise rejects with `Delivery dropped: message exceeded wire-size limits across all providers.` If no split point below the threshold is found, or if the tail is empty, recovery skips the corresponding steps. If recovery fails mid-way after the file has been truncated, the tail is restored to the file as a best-effort safeguard.
