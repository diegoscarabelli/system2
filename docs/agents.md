...
| `deliverMessage(content, details, urgent?)` | Reload knowledge, then send inter-agent message via `sendCustomMessage()`. Returns `Promise<void>` that resolves when the agent finishes processing (on `agent_end`) or rejects on permanent failure. Reload errors are swallowed to avoid dropping messages. Tracked in `pendingDeliveries` for failover replay. While a session reinit is in flight (post-scheduled-task reset or provider failover), calls queue into `pendingDeliveries` rather than rejecting — the replay paths deliver them against the new session once init completes. The uninitialized reject still fires only when `initialize()` has never been called. Each entry carries two watchdog timers (issue #194): `PENDING_DELIVERY_TIMEOUT_MS` (3 min) rejects entries that sit deferred without ever reaching the SDK (wedged-session guard); `DELIVERY_DISPATCH_TIMEOUT_MS` (6 min) rejects entries whose `sendCustomMessage` was invoked but for which `agent_end` never fired (silent SDK stream loss). Both are cleared automatically by every settlement path because the entry's `resolve`/`reject` are wrapped at push time. |
