...
**Handler timeout backstop:** every handler invoked via `trackJobExecution` is bounded by `DEFAULT_HANDLER_TIMEOUT_MS` (10 minutes, overridable per call). If the handler exceeds that window, the execution row is marked `failed` with error `'handler timed out after Xs'` and the function rejects with a `HandlerTimeoutError` so the scheduler is not blocked. The handler promise itself is detached: its later settlement is logged at `warn` if it rejects, but cannot crash the daemon or alter the already-recorded row.

The bound is enforced two ways: a `setTimeout` racing against the handler for the healthy case, and — because a wedged SDK stream can starve the event loop and delay the timer alongside the handler (issue #203) — a wall-clock check at settlement time that coerces the row to a timeout when `Date.now() - startedAt >= timeoutMs`, regardless of whether the timer or the handler won the race. The starved-loop path preserves the handler's underlying error in the message (`; handler eventually rejected: ...`) so the reason for the wedge is not lost. Note that this is a *reporting* backstop, not a cancellation mechanism: the handler continues to run until it settles on its own. Preempting a wedged handler requires either wiring `AbortSignal` down to the SDK request or running the wedge-prone work in a Worker thread; the narrower fix for the wedge itself lives in `AgentHost` (see issue #194).
