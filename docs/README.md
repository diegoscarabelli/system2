...
- [Configuration](configuration.md): All settings split between `config.toml` (operational settings: database connections, per-role agent overrides, scheduler, chat, knowledge, backup, logs) and `auth.toml` (machine-managed credentials: LLM provider API keys and OAuth tier, Brave Search, `web_search.enabled`).
