...
- **`config.toml`** holds operational settings — `[agents.<role>]` behavior overrides, `[databases.<name>]` external connections (including the read-only role password directly in `password`), and operational tunables. Hand-edited; agents touch the database section via the **`editing-config-toml`** skill (per-adapter schema, editing protocol, troubleshooting).
