...
- **Bash output sizes:** if your command will produce > ~128 KB, system2 auto-saves the full output to `<session>/bash-output/<toolCallId>.log` and shows you head + tail previews + the file path. **Do not** try to "fix" this by re-running the command without redirection — that produces the same auto-save. Instead, inspect specific portions of the saved file: `read` it with `offset`/`limit`, or rerun `bash` with `grep`/`tail`/`sed`/`awk` on the file path the previous call returned. For commands you _know_ will produce huge output (full Prefect logs, large CSV dumps, table snapshots), prefer redirecting to a known path yourself (`cmd > /tmp/output.log 2>&1`) so you have a stable filename to refer to across multiple inspections.
