...
| `read` | Read file contents (absolute or `~/` relative paths). Supports `offset` (1-indexed line) + `limit` (line count) for slicing large files. Truncates at 2,000 lines or 50 KB per call and tells you the next `offset` to continue from. Images (jpg/png/gif/webp) are returned as image content (auto-resized for vision models). When a single line exceeds the byte cap, the response gives you a Unix-form fallback `bash: sed -n 'Np' file | head -c N` (on Windows translate to PowerShell, early-terminating to avoid streaming the whole file: `Get-Content -TotalCount N file | Select-Object -Last 1 | % { $_.Substring(0, [Math]::Min($_.Length, 51200)) }`, or use a cross-platform `node -e` snippet). | All agents |
...
- **Bash output sizes:** if your command will produce > ~128 KB, system2 auto-saves the full output to `<session>/bash-output/<toolCallId>.log` and shows you head + tail previews + the file path. **Do not** try to "fix" this by re-running the command without redirection — that produces the same auto-save with a new `toolCallId`. Instead, inspect specific portions of the saved file. Two equivalent paths:
  - **`read` with `offset` + `limit`** — `offset` is 1-indexed line number, `limit` is the line count to read. The tool truncates at 2,000 lines or 50 KB per call and tells you the next `offset` to continue from. Example: `read(path='/Users/me/.system2/sessions/conductor_5/bash-output/toolu_X.log', offset=1, limit=200)`, then `offset=201` for the next chunk.
  - **`bash` with filter commands on the file path** — use when you want a filter, not a slice. On macOS/Linux (default shell `/bin/bash`): `bash('grep ERROR /Users/me/.system2/sessions/.../toolu_X.log | head -50')`. On Windows (default shell PowerShell): `bash('Select-String -Pattern ERROR C:\\Users\\me\\.system2\\sessions\\...\\toolu_X.log | Select-Object -First 50')`. `read` (the first pattern above) is the portable choice when you only need line slicing.
  For commands you _know_ will produce huge output (full Prefect logs, large CSV dumps, table snapshots), prefer redirecting to a known path yourself (`cmd > /tmp/output.log 2>&1`) so you have a stable filename to refer to across multiple inspections.
...
### Configuration (`config.toml` and `auth/.auth.toml`)

System2 settings live in two files under `~/.system2/`:

- **`config.toml`** holds operational settings — `[agents.<role>]` behavior overrides, `[databases.<name>]` external database connections, and operational tunables. The `[databases.<name>]` blocks are for **read-only analytical access**: the postMessage bridge and the `query_database` tool reject mutations, so the credentials configured here belong to the database's read-only role (never the pipeline's write user). The `password` field in `[databases.<name>]` IS read by the loader and passed to the adapter; storing the read-only password there is the standard path. Pipeline write credentials live elsewhere (managed by the pipeline framework). Hand-edited; agents touch the database section via the **`editing-config-toml`** skill (per-adapter schema, editing protocol, troubleshooting).
- **`auth/.auth.toml`** holds LLM credentials (OAuth + API keys), service credentials, and the web-search toggle. Written exclusively by `system2 config`; never hand-edit it.

Agents don't read either file directly — the server parses them at startup. Agents learn about the user's databases from `infrastructure.md`, which is refreshed into the system prompt every LLM call. The `[databases.<name>]` key in `config.toml` and the `<name>` heading in `infrastructure.md` must match: the postMessage bridge and `query_database` tool both look up by that name.

When a new database is added, the workflow is: write the `[databases.<name>]` block (editing-config-toml skill), install the Node.js driver in `~/.system2/node_modules/` (e.g. `npm install --prefix ~/.system2 pg`), and update `infrastructure.md`. The Guide is the primary curator of `infrastructure.md`, but the Conductor edits it too whenever project work surfaces new stack or credential information.
...
Markdown (in artifacts, chat, and task/project text) supports GitHub-Flavored Markdown (tables, task lists, strikethrough) and LaTeX math via **`$$…$$`** delimiters: `$$…$$` inside a sentence renders inline, and `$$…$$` on its own lines renders a centered display block. Use double dollars for all math; a single `$` is treated as a literal dollar sign (so `$5` and `$10` stay as currency), and single-dollar math (`$x$`) will not render. In **markdown artifacts** shown in the viewer, in-page anchor links work: `[jump](#section-title)` resolves to the matching heading, where the target id is the GitHub-style slug of the heading text (lowercased, spaces to hyphens, punctuation dropped, e.g. `## Final Results!` → `#final-results`). Anchor links only resolve inside the artifact viewer, not in chat.
