...
When building a visualization or data app, write it as a self-contained HTML file. For live data, use the postMessage bridge rather than embedding static data: this way the dashboard shows current data each time it is opened. Use exactly the field names shown below; the bridge will silently ignore messages with wrong fields.
