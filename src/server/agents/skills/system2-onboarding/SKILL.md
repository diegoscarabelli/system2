...
- The user oriented on the two-file config layout (`~/.system2/config.toml` for operational settings they hand-edit, `~/.system2/auth/auth.toml` for credentials managed exclusively by `system2 config`) so they know where to tune System2 later.
...
### 10. Walk through the configuration files

System2 splits its settings across two files. Orient the user on both, in this order, adapting depth to their technical level:

**`~/.system2/auth/auth.toml`** (machine-managed credentials, written exclusively by `system2 config`). Do NOT show this file as an artifact and tell the user not to hand-edit it: every `system2 config` write rewrites it via parse-mutate-stringify, so any manual edits would be lost. Instead, walk them through what `system2 config` manages on their behalf:

- **OAuth and API key tiers** (`[llm.oauth]`, `[llm.api_keys]`, with per-provider sub-sections). The two credential tiers. OAuth subscriptions (Anthropic Pro/Max, OpenAI Codex, GitHub Copilot) are tried first; API keys (Anthropic, OpenAI, Google, etc.) are the failover tier. Each tier has its own `primary` and ordered `fallback` list. Use `system2 config` to add/remove providers, add multiple labeled keys per API-key provider for rotation, set primary, and reorder fallbacks. OAuth credential JSON files live alongside `auth.toml` at `~/.system2/auth/<provider>.json` (also `0600`). Mention that System2 automatically fails over between keys and providers when errors occur.
- **Per-role model pins** (`[llm.api_keys.<provider>.models]`): a TOML table with role keys inside (e.g. `narrator = "claude-haiku-4-5-20251001"`). API-keys tier only; the OAuth resolver picks one model per provider. Managed via `system2 config`.
- **Services and the web search toggle** (`[services.brave_search]`, `[tools.web_search].enabled`): Brave Search credentials and the web-search on/off flag. Setting a Brave key auto-enables `web_search`. Managed via `system2 config`.

If the daemon is running when credentials change, restart it with `system2 stop && system2 start` to pick up the change.

**`~/.system2/config.toml`** (user-edited operational settings). Show this file (`show_artifact`) and walk them through the sections in order. This file holds operational tunables only; no credentials. Hand-editing is expected and safe: the daemon reads it but never writes it.

- **`[agents.<role>]`**: Per-role behavior overrides for any agent (guide, conductor, narrator, reviewer, worker): `thinking_level` (off/minimal/low/medium/high) and `compaction_depth` (how many context compactions before pruning). Only specified fields override the library defaults. This is how they can, for example, raise the Guide's thinking level. (Per-role *model* pins live in `auth.toml` under `[llm.api_keys.<provider>.models]` and are managed via `system2 config`.)
...
- **`web_search_max_results`** (top-level scalar): operational knob for the `web_search` tool. The on/off `enabled` flag itself lives in `auth.toml` and is managed by `system2 config`; the result-count cap is a hand-editable operational setting and lives here.
- **Operational sections** (`[backup]`, `[session]`, `[logs]`, `[scheduler]`, `[chat]`, `[knowledge]`, `[delivery]`): These exist with sensible defaults and rarely need adjustment. Point out `scheduler.daily_summary_interval_minutes` as the one users might want to tune (controls how often the Narrator writes summaries).

Keep this conversational, not a lecture. A couple of sentences per section is enough. The goal is awareness of the split (credentials via `system2 config`, operational settings hand-edited in `config.toml`) and where each knob lives, not memorization of every field.
