...
- **`[agents.<role>]`**: Per-role behavior overrides for any agent (guide, conductor, narrator, reviewer, worker): `thinking_level` (off/minimal/low/medium/high/xhigh) and `compaction_depth` (how many context compactions before pruning). Only specified fields override the library defaults. This is how they can, for example, raise the Guide's thinking level. (Per-role *model* pins live in `.auth.toml` under `[llm.api_keys.<provider>.models]` and are managed via `system2 config`.)
