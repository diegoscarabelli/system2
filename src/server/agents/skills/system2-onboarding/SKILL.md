...
- **`[llm.oauth]` and `[llm.api_keys]`** (and per-provider `[llm.oauth.<provider>]` / `[llm.api_keys.<provider>]` sub-sections): The two credential tiers. OAuth subscriptions (Anthropic Pro/Max, OpenAI Codex, GitHub Copilot) are tried first; API keys (Anthropic, OpenAI, Google, etc.) are the failover tier. Each tier has its own `primary` and ordered `fallback` list. They can add/remove providers, add multiple labeled keys per API-key provider for rotation, set primary, and reorder fallbacks via `system2 config` (no need to hand-edit). Mention that System2 automatically fails over between keys and providers when errors occur.
- **`[agents.<role>]`**: Per-role behavior overrides for any agent (guide, conductor, narrator, reviewer, worker): `thinking_level` (off/minimal/low/medium/high) and `compaction_depth` (how many context compactions before pruning). Only specified fields override the library defaults. Per-role *model* pins live separately under `[llm.api_keys.<provider>.models]` as a TOML table with role keys inside (e.g. `narrator = "claude-haiku-4-5-20251001"`); api-keys tier only — OAuth picks one model per provider via the resolver. Together this is how they can, for example, raise the Guide's thinking level or pin Mistral to use a specific model for the Narrator role.
