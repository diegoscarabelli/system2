...
- **`[agents.<role>]`**: Per-role behavior overrides for any agent (guide, conductor, narrator, reviewer, worker): `thinking_level` (off/minimal/low/medium/high) and `compaction_depth` (how many context compactions before pruning). Only specified fields override the library defaults. Per-role *model* pins live separately under `[llm.api_keys.<provider>.models][<role>]` (api-keys tier only; OAuth picks one model per provider via the resolver). Together this is how they can, for example, raise the Guide's thinking level or pin Mistral to use a specific model for the Narrator role.
