...
describe('Markdown', () => {
  it('does not assign heading ids by default (avoids duplicate ids across instances)', () => {
    const { container } = render(<Markdown>{'# Hello World'}</Markdown>);
    const heading = container.querySelector('h1');
    expect(heading?.id).toBe('');
  });

  it('assigns slug ids to headings when enableHeadingIds is set', () => {
    const { container } = render(<Markdown enableHeadingIds>{'# Hello World'}</Markdown>);
    const heading = container.querySelector('h1');
    expect(heading?.id).toBe('hello-world');
  });

  it('normalizes punctuation and case into GitHub-style slugs', () => {
    const { container } = render(<Markdown enableHeadingIds>{'## Final Results!'}</Markdown>);
  ...
  it('renders an anchor link whose href targets the generated heading id', () => {
    const { container } = render(
      <Markdown enableHeadingIds>{'[Go](#section)\n\n## Section'}</Markdown>
    );
  ...
  it('disambiguates repeated headings within a document', () => {
    const { container } = render(<Markdown enableHeadingIds>{'## Repeat\n\n## Repeat'}</Markdown>);
