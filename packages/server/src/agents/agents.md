...
- **All timestamps must be UTC ISO 8601** (e.g. `2026-03-13T16:00:00Z`). This applies to timestamps you write in files, database records, commit messages, and section headings. To get the current UTC time, use `date -u +%Y-%m-%dT%H:%M:%SZ` via bash. JSONL sessions and scheduled messages already use UTC.
