...
export function createQueryDatabaseTool(db: DatabaseClient): AgentTool<any> {
  const params = Type.Object({
    sql: Type.String({
      description: 'SQL query to execute against the app database. Tables: projects, tasks, agents.',
    }),
  });
  ...
  return {
    ...
    description: 'Execute a SQL query against the System2 app database to retrieve information about projects, tasks, and agents.',
    parameters: params,
