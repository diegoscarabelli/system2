...
2. **An LLM credential** (at least one required). Two tiers, combinable, both configurable during onboarding:
   - **OAuth subscription support** (skip API key setup if you already use one of these): use your existing AI account. Supported: [Anthropic](https://claude.com/pricing) (paid plan required), [OpenAI](https://chatgpt.com/pricing), and [GitHub Copilot](https://github.com/features/copilot/plans).
