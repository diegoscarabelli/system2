...
- Narrator daily-summary cron no longer enters a self-reinforcing context-overflow loop. Two fixes: (1) extend the post-scheduled-task session reset to error turns so a failed tick recovers instead of leaving a bloated session that grows by ~one tick's payload each retry until `handleContextOverflow` can no longer find a safe split point and manual archival is the only escape; (2) serialize scheduled-task deliveries one per SDK run via a `deferred` gate in `deliverMessage` — three deliveries per tick (project-log:N + daily-summary) were previously batched as Pi SDK `followUp` turns in a single run, so each delivery's API call carried the prior turns as conversation history and the third call's input (with the per-delivery activity budget at ~512 KB) blew the 1M-token context window. Five rounds of Copilot review plus one self-review polished invariants around the `deferred` flag: preservation across failover, FIFO ordering when chat arrives behind a deferred scheduled task (`pendingDeliveries[0..deliverySendCount-1]` must be exactly the in-flight prefix), gate-release on `sendCustomMessage` failure (no `agent_end` would otherwise fire), and gating `agent_end`'s deferred-dispatch off errored turns and `isReinitializing` state to avoid racing with `reinitializeWithProvider`'s replay loop. Fixes [#189](https://github.com/diegoscarabelli/system2/issues/189).
