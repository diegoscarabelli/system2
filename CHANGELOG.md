...
- OAuth refresh-retry failures now surface an actionable hint ("Run `system2 config` and restart the server to re-authenticate.") in the user-visible chat error, instead of failing silently into fallback providers. Background: an OAuth grant can be revoked server-side (typical after a user signs out of Claude.ai or the session is administratively expired) without the refresh endpoint being aware. The refresh call still mints a new access token, but every API call with that token 401s. `AgentHost.handlePotentialError` already runs a one-shot refresh-and-retry on the first 401 of a delivery; when the retried request also 401s, the second invocation finds the per-delivery guard already set, skips the refresh branch, and falls through to standard cooldown failover. The chat message at that point read "401 auth_error, all providers unavailable on anthropic ... Invalid authentication credentials" with no indication that the fix is to re-authenticate manually. Two changes: (1) `oauthRefreshAttempted` is now `oauthRefreshAttemptedFor: Set<LlmProvider>` — the previous single boolean conflated state across providers (a refresh-retry on anthropic would have suppressed openai-codex's chance to refresh-retry on a later 401, and would have triggered the new hint for any 401 even on a never-attempted provider); per-provider tracking gates the hint to the specific provider whose automatic recovery has run; (2) the new `oauthReauthHintFor(provider)` helper appends the hint to every failover/all-unavailable chat-message path in `handlePotentialError` (cooldown-by-another-agent switch/rotate, post-`markKeyFailed` failover, all-providers-unavailable terminal, and last-resort failover). The set clears on successful delivery (`agent_end`), so once the user re-auths and a delivery succeeds, the hint stops appearing. Symptom that prompted the fix: 2026-05-15 ~21:29 UTC the anthropic OAuth grant had been broken since 2026-05-11 but was masked by fallback to openai-codex until both providers happened to be revoked simultaneously; the catch-up backlog had grown to 65h of daily summaries and 75h of memory updates before the failure became visible. Out of scope: API-key 401 messaging (different tier, separate UX concern) and startup auth ping/banner UI (explicitly rejected — reuse the existing chat error path). Fixes [#188](https://github.com/diegoscarabelli/system2/issues/188).
