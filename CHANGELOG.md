...
- `ArtifactViewer`'s iframe no longer runs away when an artifact uses viewport-relative units (`vh`, `%`) on a plot container. Previously the `ResizeObserver` that synced iframe height to content would grow the iframe → grow `vh` inside → grow scrollHeight → fire the observer again, ending in Chromium's silent `ResizeObserver loop limit exceeded` guard freezing the iframe at some absurd height (Plotly colorbars stretched to fill the viewport, choropleths squished off-screen). The iframe now defaults to filling its panel (`height: 100%`) so `vh` resolves to the panel's stable height, and the observer only grows past the panel when scrollHeight genuinely exceeds it (long reports, database viewer after query results arrive) — the parent panel scrolls, preserving the routing that keeps the CSS invert filter off the scrollbar. A 100 ms throttle catches remaining feedback (any scrollHeight growth within that window of a size-set is skipped) and a one-shot `console.warn` fires when the guard trips so the failure stops being silent. See [#204](https://github.com/diegoscarabelli/system2/issues/204).
- The scheduler's handler-timeout backstop no longer silently misses under event-loop starvation. `trackJobExecution` bounds each Narrator scheduled-task handler with a `setTimeout` that races against the handler's own promise, but when a wedged Anthropic SDK stream starved the Node event loop (issue [#203](https://github.com/diegoscarabelli/system2/issues/203)) the timer fired late alongside the handler's own settlement, so `Promise.race` was won by the handler's `AggregateError` and the intended `HandlerTimeoutError` never reached the DB row. On 2026-07-21 this produced seven consecutive `daily-summary` rows in the 1000–1073s band, all reporting `3/3 deliveries failed` instead of a timeout. The bound is now enforced two ways: the existing `setTimeout` for the healthy case, plus a wall-clock re-check at settlement — if `Date.now() - startedAt > timeoutMs`, a *rejected* handler is coerced to `HandlerTimeoutError` (including `JobSkipped` past the deadline, since a 15-minute "skip" is by definition a wedge), while a *resolved* handler stays `completed` with a warn log (side effects like `advanceFrontmatterCursors` have already committed, so coercing to `failed` would desync the row from disk state). The underlying error is stashed as ES2022 `cause` on the thrown timeout so `util.inspect`, `console.error`, and DB-row post-mortems all follow the wedge trail; `AggregateError.errors` is flattened into the message so per-delivery reasons survive on the row. A `STARVATION_TOLERANCE_MS` (100 ms) absorbs normal Node timer jitter so healthy clean timeouts are no longer mis-tagged as event-loop starvation. Croner's `safeHandler` branches on `HandlerTimeoutError` to log at `warn` with a distinct label so log-based alerting on genuine `handler error (uncaught)` isn't diluted. This is a reporting fix, not preemption — the handler continues to run until it settles on its own; preempting a wedged handler requires Worker threads or wiring `AbortSignal` down to the SDK request, and the wedge itself is issue [#194](https://github.com/diegoscarabelli/system2/issues/194).

### Changed

- `HandlerTimeoutError` constructor is now `(elapsedMs, deadlineMs, cause?)` (previously `(timeoutMs)`). The old public `.timeoutMs` field is replaced by `.elapsedMs`, `.deadlineMs`, and `.starved`; the underlying reason is reachable via the ES2022 `.cause` idiom, with `.underlyingError` retained as a getter over `.cause` for backward compatibility. No in-repo callers depended on `.timeoutMs`. Healthy timer-fire messages stay `handler timed out after Xs`; starved-loop coercion messages become `handler timed out after Xs (deadline Ys; event loop was starved past the timer); handler eventually rejected: <underlying> [flattened AggregateError reasons]`.
...
- The Narrator daily-summary cron no longer wedges itself into a permanent failure loop. The Narrator truncates its session to a fresh header after every scheduled-task delivery (its durable memory lives in files, not the session) — but only on a clean `agent_end`. When a delivery left no clean `agent_end` (a wedged turn, or a context-overflow recovery loop whose split point is blind to the large injected `custom_message` deliveries it leaves in the restored tail), the reset never fired, so each cron tick appended another scheduled-task payload to the session JSONL until it blew past the model's context window and every delivery failed with "prompt is too long" (observed at ~26M tokens / 43 MB, with empty daily summaries). `AgentHost.reclaimBloatedSession()` now runs at the start of each scheduled-task cycle (`buildAndDeliverDailySummary`, `buildAndDeliverMemoryUpdate`) and archives an oversized carried-over session to a fresh header before delivering, bounding session growth independently of `agent_end`.
- Context-overflow recovery (`handleContextOverflow`) no longer loops forever on oversized injected deliveries. It chooses its split point from message-level token `usage`, which `custom_message` deliveries don't carry, so a tail packed with large deliveries stayed over the window and re-overflowed on every attempt (600+ recoveries in the incident above, never converging). Recovery now drops the tail when it exceeds a byte budget and recovers from the compacted head alone — a clean under-window recovery instead of restoring a tail that re-overflows. This affects all agents, not just the Narrator.
- An inter-agent delivery that wedges (dispatched to the SDK but never produces `agent_end`) now clears the agent's `busy` flag when its dispatch watchdog fires. Previously `busy` was set at dispatch and only cleared by `agent_end`, so a wedged delivery left the agent appearing busy indefinitely (issue #194).
...
- `AgentHost.deliverMessage` now wraps each delivery in two watchdog timers so the wedged-session failure mode from issue [#194](https://github.com/diegoscarabelli/system2/issues/194) cannot hang the daemon. (1) `PENDING_DELIVERY_TIMEOUT_MS` (3 minutes) rejects deliveries that sit in `pendingDeliveries` deferred without ever reaching `sendCustomMessage` — the gate branch defers a delivery whenever `this.session` is null or `isReinitializing` is true (also for FIFO and scheduled-task gating), and if the expected reinit / replay never runs the entry would otherwise hang indefinitely. (2) `DELIVERY_DISPATCH_TIMEOUT_MS` (6 minutes) rejects deliveries whose `sendCustomMessage` was invoked but for which `agent_end` never fired — guards against silent SDK stream loss (no error event, no end event, dead socket). On dispatch timeout the timer also decrements `deliverySendCount`, clears `currentTurnHasOutput`, and dispatches the next deferred entry via `replayPendingDeliveries`, so a single hung dispatch cannot permanently block the FIFO. Both timeouts are compile-time constants exported for tests, not runtime-configurable. Both timer handles are stored on the pendingDeliveries entry; the entry's `resolve`/`reject` are wrapped at push time to clear them on settlement, so every existing cleanup path (agent_end shift, failover replay, contamination guard, wire-size overflow, etc.) tears down the timers without per-site changes. Pairs with the universal scheduler backstop from v0.3.13.
...
- OAuth re-auth hint (#188) now actually reaches the chat on Anthropic 401s, and names the provider. Two issues with the v0.3.11 behavior. (1) The contamination guard in `handlePotentialError` flipped on `message_start` — but that event fires whenever a message scaffold is created, including for assistant streams that abort with an auth failure before any token arrives. Anthropic's streaming endpoint opens the stream, fires `message_start`, then closes with 401, so the guard incorrectly classified the turn as contaminated and dropped pending deliveries before the failover path could write a user-visible system message. Narrowing the flag to flip only on `message_update` (real token streaming) or `tool_execution_start` (real side effects) preserves the #175 protection while letting `message_start`-then-401 flow through the normal failover path. (2) The hint string didn't name the provider; with multiple OAuth providers configured, "re-authenticate" was ambiguous. Now reads "Run \`system2 config\` to refresh `<provider>` authentication and restart the server" — e.g. *"switching from anthropic to openai-codex. Run \`system2 config\` to refresh anthropic authentication and restart the server."* Fixes [#192](https://github.com/diegoscarabelli/system2/issues/192).

## [0.3.11] - 2026-05-16

### Fixed

- Narrator daily-summary cron no longer enters a self-reinforcing context-overflow loop. Two fixes ship together. (1) The post-scheduled-task session reset now fires on error turns too. Without it, a failed tick left a bloated session that grew by ~one tick's payload each retry. Eventually `handleContextOverflow` couldn't find a safe split point, and manual archival was the only way out. (2) Scheduled-task deliveries are now serialized one per SDK run via a `deferred` gate in `deliverMessage`. The scheduler queues three deliveries per tick (project-log:N + daily-summary). The Pi SDK previously batched them as `followUp` turns in a single run, so each call carried the prior turns as conversation history. With the per-delivery activity budget at ~512 KB, the third call's input blew the 1M-token context window. Five rounds of Copilot review plus one self-review polished invariants around the `deferred` flag. The four cases: preservation across failover; FIFO ordering when chat arrives behind a deferred scheduled task (the queue's first `deliverySendCount` entries must be exactly the in-flight prefix); gate-release on `sendCustomMessage` failure (no `agent_end` would otherwise fire); and skipping `agent_end`'s dispatch on errored turns and `isReinitializing` state, which would otherwise race with `reinitializeWithProvider`'s replay loop. Fixes [#189](https://github.com/diegoscarabelli/system2/issues/189).
...
- UI WebSocket now auto-reconnects after the connection drops, instead of getting stuck on "Connecting to server..." until the user reloads the page. The hook in `src/ui/hooks/useWebSocket.ts` previously opened a single socket on mount, and its `onclose` handler only flipped `connected = false`; the surrounding `useEffect` had `[]` deps so the socket was never reopened. The reconnect plumbing on the server-handshake side (clearing stale `agent_busy` state and bumping push counters on `hasConnectedOnce`) was already in place but unreachable because nothing ever triggered a second `onopen`. Most reproducible after macOS display sleep: the OS suspends network activity long enough that either the browser, the server, or an intermediary closes the socket, and on wake the UI sat on a dead connection. The fix wraps the socket setup in a `connect()` closure inside the same `useEffect`, schedules a reconnect from `onclose` with exponential backoff (1s, 2s, 4s, 8s, 16s, capped at 30s) tracked via a closure-scoped `reconnectAttempts` counter that resets on successful `onopen`, and adds `visibilitychange` + `online` listeners that call a `reconnectNow()` helper to retry immediately when the tab becomes visible or the network comes back (skipping the wait for the next backoff tick). `reconnectNow` is a no-op when the socket is already `OPEN`/`CONNECTING` to avoid stomping a healthy in-flight connection. Five rounds of Copilot review surfaced a chain of subtle stale-socket races worth recording: (1) `reconnectNow()` can create a new socket while the previous one is still `CLOSING`, and the old socket's `onclose`/`onerror` would still fire asynchronously after `wsRef.current` had been replaced, flipping `isConnected=false` on the new socket and scheduling an extra reconnect that would create a third concurrent socket (2) `onopen` on a superseded socket could similarly flip `isConnected=true` and send a `switch_agent` on a dead connection; `onmessage` could mutate the chat/push stores using outdated agent state during the close-handshake window; (3) the very `unmounted` guard introduced to fix (1) and (2) then short-circuited `onclose` during component teardown, leaving the chat store with `isConnected=true` + uncleared streaming state forever after StrictMode dev double-mounts, HMR reloads, or any future route change. The shipped version guards `onopen`/`onmessage`/`onerror`/`onclose` with `if (unmounted || wsRef.current !== ws) return;` (the captured `ws` identifies superseded sockets), clears any pending `reconnectTimer` in `onopen` so a successful connection doesn't leave a scheduled retry alive, and applies `setConnected(false)` + `clearAllStreamingState()` directly in the `useEffect` cleanup to compensate for the guarded `onclose` during teardown. Per-PR test additions targeted the new store action only; vitest coverage for the reconnect/backoff hook itself was deferred (a mocked-WebSocket + fake-timer harness would be more cost than value for a hook this small, and behavior is observable end-to-end via lid sleep / airplane mode / daemon restart). ([#187](https://github.com/diegoscarabelli/system2/pull/187))

### Added

- Per-agent message-input drafts. The chat input previously held its unsent text in a local `useState` inside `MessageInput`, so switching agents kept the previous agent's draft visible (and would send it to the new agent on Enter). Lift the draft into per-agent state: add `inputDraft: string` to `PerAgentState` (default `''`) and a `setInputDraft(value, agentId?)` action keyed by `agentId`, then read/write the textarea value through the store via the active agent's draft. Switching agents swaps in that agent's saved draft (empty if none); switching back restores the original text. The textarea auto-resize now runs in a `useEffect` on `activeAgentId` so the height matches the new draft instead of inheriting the previous agent's height (`onChange` would otherwise be the only path that calls `autoResize`). Drafts are intentionally **not** persisted across page reload — the chat store's `persist` `partialize` already restricts localStorage to `activeAgentId` / role / label, and `inputDraft` lives inside `agentStates` which is excluded. Same behavior as before for reloads; the change is purely scoped to in-tab agent switches. Four store-level tests in `src/ui/stores/chat.test.ts` lock in the isolation contract (agent A's draft doesn't leak to agent B; clearing on submit only clears the active agent; `setInputDraft` is a silent no-op when no agent is active). ([#187](https://github.com/diegoscarabelli/system2/pull/187))

## [0.3.9] - 2026-05-12

### Fixed

- Auto-compaction no longer surfaces a phantom "Context compaction failed: Auto-compaction failed: Cannot read properties of undefined (reading 'signal')" after a successful compaction. Pi-coding-agent's `AgentSession._runAutoCompaction` stores its `AbortController` on a single `this._autoCompactionAbortController` field at the top of its `try` block and clears it in `finally`. The SDK has two independent async entry points into `_checkCompaction`: the event-queue path on every `agent_end` (line 343 in `agent-session.js`) and the pre-send path inside `AgentSession.prompt()` (line 758). The event queue serializes its own calls but does not synchronize with `session.prompt()` callers — and `session.prompt()` is invoked from system2 on every WebSocket user message and from the failover-retry paths in `AgentHost.handlePotentialError` (`host.ts:1056`/`1399`). When a user message or a failover retry arrives while a threshold-driven compaction from the previous turn's `agent_end` is still awaiting the `compact()` LLM call, the second `_runAutoCompaction` overwrites `this._autoCompactionAbortController`. Whichever run finishes first clears the field in its `finally`; the other then reaches `if (this._autoCompactionAbortController.signal.aborted)` after its own `compact()` resolves, reads `.signal` on `undefined`, throws `TypeError`, and the catch wraps that into a `compaction_end` with `errorMessage: "Auto-compaction failed: Cannot read properties of undefined (reading 'signal')"`. The compaction itself had succeeded; only the post-run abort check crashed, but `history-capture` surfaced the misleading "failed" message into the chat timeline. Observed live in `guide_1`'s chat-cache: 3 overlapping `compaction_start` events at 08:50:19 / 08:50:26 / 08:51:07 followed by 1 "Context compacted" success and 2 "signal" failures. Patched locally via `pnpm patch @mariozechner/pi-coding-agent@0.71.1` with three defenses in `_runAutoCompaction`: (1) a guard at the top that bails silently when another run is in flight, so no spurious start/end pair is emitted; (2) a local `abortController` variable used for every `.signal` access inside the run, immune to field overwrite; (3) a conditional `finally` clear that only resets the field when it still points at our controller, so a future caller that bypasses the guard cannot wipe a newer run's controller. `isCompacting` and `abortAutoCompaction()` continue to read/abort the same field and remain correct. Trade-off: an overflow-recovery call (`willRetry: true`) that arrives during a threshold compaction now skips silently instead of running concurrently, so its auto-retry `setTimeout` does not get scheduled — but this case was already crashing with the same `TypeError`, and the in-flight compaction still reduces context, so the user's next prompt typically succeeds without manual retry ([#186](https://github.com/diegoscarabelli/system2/pull/186)).
